...
def build_todays_player_df(game_date: str | None = None, current_round: int = 1) -> pd.DataFrame:
    ...
    # Team commitment: remaining ≥20 RS PRA players (not yet picked) / total ≥20 RS PRA
    ...
    for pid, team in pid_to_team.items():
        ...
        if rs_avg >= 20:
            ...
            if pid not in used_ids:
                remaining_per_team[team] = remaining_per_team.get(team, 0) + 1
    df["team_picks_used"] = df["Team"].map(lambda t: remaining_per_team.get(t, 0))
...
def _render_table_from_store(store_data, urgency_field, hide_used=False):
    ...
    col_defs = [
        ...
        {"field": "Pos"},
        {"field": "Team",
         "valueGetter": {"function": "params.data['Team'] + (params.data['Series Win%'] != null ? ' ' + (params.data['Series Win%']*100).toFixed(0)+'%' : '')"},
         "cellStyle": {"function": "var p=params.data['Series Win%']; var dim=(params.data.Display_Status&&(params.data.Display_Status[0]==='✓'||params.data.Display_Status[0]==='❌')); return p==null?{}:{color: dim?'#aaa':p>0.6?'#16a34a':p>0.4?'#ca8a04':'#dc2626'};"}},
